...
If a single-variable function $f:U \subseteq \R$ is differentiable at $x_0 \in U$, then the limit
...
\end{defn}
The converse is also true: if $\Delta f=T+r$, where $T$ is a linear map (which must be of the form $T(h)=ch$ for some constant $c$) and $\lim_{h\to 0}\frac{r(h)}{h}$, then
\begin{align*}
...
\begin{enumerate}[(1)]
	\item $d$ is linear: $d(cf_1+f_2)_\bx=c(df_1)_\bx+(df_2)_\bx$.
	\item $d$ obeys the Leibnitz rule: $d(f_1 \cdot f_2)_\bx=(df_1)_\bx \cdot f_2(\bx)+f_1(\bx) \cdot (df_2)_\bx$.
	\item $d$ obeys the quotient rule: $d(\frac{f}{g})_\bx=\frac{g(\bx) df_\bx-f(\bx)dg_\bx}{(g(\bx))^2}$.
\end{enumerate}	
\end{thm}
...
These all follow from the corresponding properties of the directional derivative, which are familiar from single-variable calculus.
	\begin{enumerate}[(1)]
		...
		\end{align*}
		and therefore $d(cf_1+f_2)_\bx=c(df_1)_\bx+(df_2)_\bx$.
		\item For any $\bv \in \R^n$, we have
		\begin{align*}
		...
		\end{align*}
		and therefore $d(f_1 \cdot f_2)_\bx=(df_1)_\bx \cdot f_2+f_1 \cdot (df_2)_\bx$.
		\item For any $\bv \in \R^n$, we have
		\begin{align*}
			d(\frac{f}{g})(\bv)&=D_\bv(\frac{f(\bx)}{g(\bx)})=\frac{g(\bx)D_\bv(f)-f(\bx)D_\bv(g)}{(g(\bx))^2}=\frac{g(\bx) df_\bx(\bv)-f(\bx)dg_\bx(\bv)}{(g(\bx))^2},
		\end{align*}
		and therefore $d(\frac{f}{g})_\bx=\frac{g(\bx) df_\bx-f(\bx)dg_\bx}{(g(\bx))^2}$.
	\end{enumerate}
\end{proof}
...
If $f:\R^2 \to \R$ is differentiable at $\bx$, the equation of the \emph{tangent plane} to the graph of $f$ at $(\bx,f(\bx))$ is given by the equation
\begin{align*}
	z=z_0+\frac{\partial f(x_0,y_0)}{\partial x}(x-x_0)+\frac{\partial f(x_0,y_0)}{\partial y}(y-y_0).
\end{align*}

\fixme{Add examples from the book.}
