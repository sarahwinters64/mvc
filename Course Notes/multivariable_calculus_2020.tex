...
Let $f(x)$ be defined on an open subset $U$ of $\R$. If $f$ is differentiable on $U$, then
\begin{align}\label{eq:old_derivative}
	f'(x)=\lim_{h \to 0}\frac{f(x+h)-f(x)}{h}
\end{align}
exists for each $x \in U$. Rearrange \eqref{eq:old_derivative} as follows:
\begin{align*}
	\lim_{h \to 0}\left[\frac{f(x+h)-f(x)}{h}\right]-f'(x)&=0 \\
	\lim_{h \to 0}\left[\frac{f(x+h)-f(x)}{h}-f'(x)\right]&=0 \\
	\lim_{h \to 0}\left[\frac{f(x+h)-f(x)}{h}-\frac{f'(x)h}{h}\right]&=0 \\
	\lim_{h \to 0}\left[\frac{f(x+h)-f(x)-f'(x)h}{h}\right]&=0 \\
\end{align*}
Let $r(h) \equiv f(x+h)-f(x)-f'(x)h$. Then existence of the limit in equation \eqref{eq:old_derivative} implies that 
\begin{align}
	f(x+h)=f(x)+f'(x)h+r(h)
\end{align}
where $\lim_{h \to 0} \frac{r(h)}{h}=0$. This says that, up to a small error term, $f(x)$ is an affine map (see Appendix \ref{app:affine_space}) with differential 
\begin{align*}
	df_x:\R &\to \R \\
	df_x(h)&=f'(x)h.
\end{align*}
That is, $df_x$ is the linear function which multiplies $h \in \R$ by the derivative of $f$ at $x$. \fixme{Refer to Linear Algebra notes. We have seen that every linear map from $\R \to \R$ is multiplication by some fixed real number.}

We now give an equivalent definition of differentiability which will readily generalize to multivariable functions. We will say that $f(x)$ is \emph{old style differentiable} at $x$ if 
\begin{align*}
	f'(x)=\lim_{h \to 0}\frac{f(x+h)-f(x)}{h}
\end{align*}
exists.

\begin{defn}[Differentiable function]\label{def:differentiable_function}
	A function $f:U \subseteq \R \to \R$ defined on an open subset $U$ of $\R$ is \emph{differentiable} at $x \in U$ if there exists a linear map $T:\R \to \R$ such that 
	\begin{align*}
		f(x+h)=f(x)+Th+r(h)
	\end{align*}
...
\begin{prop}
	If $T,T':\R \to \R$ are two linear maps which satisfy Definition \ref{def:differentiable_function}, then $T'=T$.
\end{prop}

\begin{proof}
	Suppose 
	\begin{align*}
		f(x+h)&=f(x)+Th+r(h), \text{ and } \\
		f(x+h)&=f(x)+T'h+r'(h) \\
	\end{align*}
	where $\lim_{h \to 0}\frac{r(h)}{h}=0$ and $\lim_{h \to 0}\frac{r'(h)}{h}=0$. Then
	\begin{align*}
		f(x)+Th+r(h)=f(x)+T'h+r'(h)
	\end{align*}
	implies
	\begin{align*}
		T-T'=\frac{r'(h)}{h}-\frac{r(h)}{h}
	\end{align*}
	and therefore
	\begin{align*}
		T-T'=\lim_{h \to 0}(T-T')=\lim_{h \to 0}\left(\frac{r'(h)}{h}-\frac{r(h)}{h}\right)=\lim_{h \to 0}\frac{r'(h)}{h}-\lim_{h \to 0}\frac{r(h)}{h}=0+0=0,
	\end{align*}
	hene $T=T'$.
\end{proof}

\begin{defn}[Differential]
	If $f$ is differentiable at $x$, we call the unique linear map $T$ in Definition \ref{def:differentiable_function} the \emph{differential of $f$ at $x$} and write $T=df_x$.
\end{defn}

We have seen above that if $f$ is old style differentiable, then it is differentiable in the sense of Definition \ref{def:differentiable_function}. Conversely, if $f$ is differentiable at $x$ then there exists a unique linear map $T$ such that  
\begin{align*}
		f(x+h)=f(x)+Th+r(h),
	\end{align*}
which implies that 
\begin{align*}
	T=\frac{f(x+h)-f(x)-r(h)}{h}
\end{align*}
and therefore
\begin{align*}
	T=\lim_{h \to 0}T=\lim_{h \to 0}\left(\frac{f(x+h)-f(x)}{h}-\frac{r(h)}{h}\right)=\lim_{h \to 0}\frac{f(x+h)-f(x)}{h}-\lim_{h \to 0}\frac{r(h)}{h}=f'(x)+0=f'(x),
\end{align*}
so $f$ is old style differentiable at $x$.

\subsubsection{Differential of a Multivariable Function}
Following the previous discussion, we will define a multivariable function to be differentiable if it is well-approximated by an affine map, up to a small error term.

\begin{defn}[Differentiable multivariable function]\label{def:differentiable_mv_function}
	Le $f:U \subseteq \R^n \to \R$ be defined on an open subset of $\R^n$. We say that $f$ is \emph{differentiable} at $x \in U$ if there exists a linear map $T:\R^n \to \R$ such that 
	\begin{align*}
		f(\bx+\bh)=f(\bx)+T\bh+r(\bh)
	\end{align*}
	where $\lim_{\bh \to 0}\frac{|r(\bh)|}{||\bh||}=0$. Equivalently, $f$ is differentiable at $x \in U$ if for every $\epsilon>0$ there exists $\delta>0$ such that $||\bh||<\delta$ implies
	\begin{align*}
		|f(\bx+\bh)-f(\bx)-T\bh|<\epsilon ||\bh||.
	\end{align*}
	
\end{defn}

\begin{defn}[Differential of a multivariable function]
	If $f$ is differentiable at $\bx$, we call the unique linear map $T$ in Definition \ref{def:differentiable_mv_function} the \emph{differential of $f$ at $\bx$} and write $T=df_\bx$.
\end{defn}

\begin{prop}[Uniqueness of the differential]
...
\begin{proof}
	...
	\begin{align*}
		...
		&=|T-(f(\bx+\bh)-f(\bx))+f(\bx+\bh)-f(\bx)-T'| \\
		&\leq |T-(f(\bx+\bh)-f(\bx))|+|f(\bx+\bh)-f(\bx)-T'| \\
		&= |f(\bx+\bh)-f(\bx)-T|+|f(\bx+\bh)-f(\bx)-T'| \\
		&=|r(\bh)|+|r'(\bh)|
	\end{align*}
	and therefore
\begin{align*}
...
\begin{remark}
\begin{enumerate}[(a)]
	\item If $f:A \to B$ is an affine map (Definition \ref{def:affine_map}), then $f$ is differentiable for all $p \in A$ and $df_p$ is independent of $p$. In other words, an affine map has a constant differential. 
	\item If $f$ is a linear map, then $d_p=f$.
	\item If $f:U \to B$ is differentiable at $p \in U$, then the differential is a map
	\begin{align*}
		df:U \to \Hom(V,W).
	\end{align*}
\end{enumerate}	
...
As in the single variable case, a differentiable function $f:U\subset \mathbb{R}^n \to \mathbb{R}$ is continuous.
...
\begin{proof}
	Let $df_\bx$ be the differential of $f$ at $\bx$ and let $\epsilon>0$.
	\begin{align*}
		|f(\bx + \bh)-f(\bx)|&=|f(\bx + \bh)-f(\bx)-df_x(\bh)+df_x(\bh)| \\
		&\leq |f(\bx + \bh)-f(\bx)-df_x(\bh)|+|df_x(\bh)|.
	\end{align*}
	Since $f$ is differentiable, there exists $\delta_0>0$ such that $|f(\bx + \bh)-f(\bx)-df_x(\bh)|<\frac{\epsilon}{2}||\bh||$ whenever $||\bh||<\delta_0$. By Theorem \ref{thm:every_linear_map_on_a_fd_nls_is_bdd}, there exists $C>0$ such that $|df_x(\bh)|<\epsilon C||\bh||$. Thus
	\begin{align*}
		|f(\bx + \bh)-f(\bx)|&\leq |f(\bx + \bh)-f(\bx)-df_x(\bh)|+|df_x(\bh)| \\
		&<\epsilon(1+C)||\bh||. 
	\end{align*}
	Let $\delta=\min\{\delta_0,\frac{1}{(1+C)}\}$. Then for $||\bh||<\delta$ implies 
	\begin{align*}
		|f(\bx + \bh)-f(\bx)|&\leq \epsilon(1+C)||\bh|| \\
		&<\epsilon(1+C)\frac{1}{(1+C)} \\
		&=\epsilon,
	\end{align*}
	hence $f$ is continuous at $\bx$.
\end{proof}

\begin{defn}[Continuously differentiable function]
	If $f$ is differentiable on $U$ and $df:U \to \Hom(V,W)$ is continuous, then we say that $f$ is \emph{continuously differentiable}.
\end{defn}


Note that the existence of both partial derivatives of a function $f$ at $(x,y)$ is \emph{not} sufficient for $f$ to be continuous at $(x,y)$.
...
\end{figure}
\end{example}
Thus, the existence of both partial derivatives at $(x,y)$ is not strong enough to guarantee continuity at $(x,y)$. This also makes intuitive sense: continuity at $(a,b)$ requires that $\lim_{(x,y) \to (a,b)}f(x,y)=f(a,b)$ when the limit is taken along \emph{any} path passing through $(a,b)$. The partial derivatives, on the other hand, only depend on what is happening along lines through $(a,b)$ parallel to the $x$ and $y$ axes, and therefore they failed to detect the discontinuity in the previous example.

\subsubsection{Matrix Representation of the Differential}
In this section we clarify the relation between the differential of a function $f$ and the partial derivatives of $f$.

Let $f:\R^n \to \R$. Then $df_\bx$ is a linear map for each $\bx \in \R^n$, so it can be represented by a matrix \fixme{Refer to my Linear Algebra notes.}
  
\begin{thm}
	Let $U$ be an open subset of $\mathbb{R}^n$ and $f:U \to \mathbb{R}$. If $f$ is differentiable at $\bx \in U$, then the partial derivatives $\frac{\partial f}{\partial x_j}(\bx)$ all exist, and 
	\begin{align*}
		df_\bx({\bf e}_j)=\sum_{i=1}^m \frac{\partial f}{\partial x_j}(\bx), \ (1 \leq j \leq n),
	\end{align*}
	where $\{{\bf e}_j\}_{j=1}^n$ denotes the standard basis for $\mathbb{R}^n$.
\end{thm}

\begin{pf}
	The $j$th column of the standard matrix of $df_\bx$ is $df_\bx({\bf e}_j)$. Since, $f$ is differentiable at $\bx$,
	\begin{align*}
		f(\bx+t{\bf e}_j)-f(\bx)=df_\bx(t{\bf e}_j)+r(t{\bf e}_j)
	\end{align*}
	where $r(t{\bf e}_j)/t \to 0$ as $t \to 0$. Since $df_\bx$ is linear, $df_\bx(t{\bf e}_j)=tdf_\bx({\bf e}_j)$. Dividing both sides of the equation above by $t$ and taking $t \to 0$, we have 
	\begin{align*}
		\frac{\partial f}{\partial x_j}(\bx)\equiv\lim_{t \to 0}\frac{f(\bx+t{\bf e}_j)-f(\bx)}{t}&=\lim_{t \to 0}[df_\bx({\bf e}_j)+\frac{r(t{\bf e}_j)}{t}] \\
		&=\lim_{t \to 0}df_\bx({\bf e}_j)+\lim_{t \to 0}\frac{r(t{\bf e}_j)}{t} \\
		&=df_\bx({\bf e}_j)+0\\
		&=df_\bx({\bf e}_j).
	\end{align*}
\end{pf}	
...
\begin{defn}
	The $1 \times n$ matrix representing $df_\bx$ is called the \emph{gradient of $f$ at $\bx$}, and is denoted by
	\begin{align*}
		\nabla f(\bx)=(\frac{\partial f}{\partial x_1}(\bx),\frac{\partial f}{\partial x_2}(\bx),\cdots,\frac{\partial f}{\partial x_n}f(\bx)).
	\end{align*}
\end{defn}

\begin{example}
	\fixme{Choose a function which is not linear.}
	Let $f(x,y)=3x+2y$. Then $\nabla f(x,y)=(3,2)$ and therefore
	\begin{align*}
		df_{(x,y)}(h_1,h_2)&=\nabla f(x,y)\cdot (h_1,h_2) \\
		&=3h_1+2h_2 \\
		&=f(h_1,h_2).
	\end{align*}
\end{example}
...
As we saw in Example \ref{ex:p_d_but_disc}, the existence of all partial derivatives is not enough to guarantee differentiability (their existence was not even enough to guarantee continuity), that is, the existence of the differential $df$. However, as we will now show, if the partial derivatives all exist and are \emph{continuous}, then the differential \emph{does} exist. Thus, {\bf any function whose partial derivatives exist and are continuous is differentiable in the sense of Definition \ref{def:differentiable_mv_function}}.
...
\begin{example}
	Let $f:\mathbb{R}^3 \to \mathbb{R}$ be defined by $f(x,y,z)=3x^2y^3z$. Since the partial derivatives
	\begin{align*}
		\frac{\partial f}{\partial x}(x,y,z)=6xy^3z, \hspace{0.25cm} \frac{\partial f}{\partial y}(x,y,z)=9x^2y^2z, \hspace{0.25cm} \frac{\partial f}{\partial z}(x,y,z)=3x^2y^3
	\end{align*}
	are continuous, the function $f$ is differentiable with gradient
	\begin{align*}
		\nabla f(x,y,z)=\begin{bmatrix}6xy^3z & 9x^2y^2z & 3x^2y^3 \end{bmatrix}.
	\end{align*}
\end{example}
 
\fixme{Write equation of tangent plane at $(x,y)$ for a differentiable function $f(x,y)$.}

\fixme{Directional derivatives here?}

\subsection{The Composite Mapping Theorem (Chain Rule)}
Suppose a particle moves along a path $C$ along which the temperature varies. The path is the image of 
\begin{align*}
	\bbr:I \to \mathbb{R}^2
\end{align*}
while the temperature at a point $(x,y) \in C$ is $z=T(x,y)$. Thus, $T:C \subset \mathbb{R}^2 \to \mathbb{R}$. The composite function
\begin{align*}
	F\equiv T \circ \bbr:I \to \mathbb{R}
\end{align*}
defined by $F(t)=T(\bbr(t))$
gives the temperature felt by the particle at time $t$. Then $F'(t)$ is the rate at which the temperature at the position of the particle is changing with time at $t \in I$. How do we compute $F'(t)$?
