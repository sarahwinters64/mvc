...
\begin{thm}[Limit laws for vector-valued functions]
	Let $\bu, \bv$ be vector valued functions defined on the same open interval containing $t_0$ and let $c \in \R$ be a constant. Then
	\begin{enumerate}[(i)]
...
\begin{thm}[Limit law for cross product]
	If $\bu,\bv$ are vector-valued functions into $\R^3$, then
	\begin{align*}
		\lim_{t \to t_0}[\bu(t)\times\bv(t)]=\lim_{t \to t_0}\bu(t)\times\lim_{t \to t_0}\bv(t).
	\end{align*}
\end{thm}
