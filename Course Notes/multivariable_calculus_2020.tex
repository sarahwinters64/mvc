...
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.5]{figures_mvc/lagrange_1}
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.4]{figures_mvc/lagrange_2}
...
\section{Continuous Nonzero Maps}\label{sec:continuous_nonzero_maps}
\begin{thm}
If $f:U \subseteq \R^n \to \R$ is continuous and positive at $\bx_0$, then $f(\bx) > 0$ throughout an open disc centered at $\bx_0$.
\end{thm}

\begin{proof}
	Let $\epsilon=\frac{f(\bx_0)}{2}$, which is positive by assumption. Since $f$ is continuous at $\bx_0$, there exists $\delta>0$ such that 
	\begin{align}\label{eq:cont_condition}
		|f(\bx)-f(\bx_0)|<\epsilon=\frac{f(\bx_0)}{2}
	\end{align}
	whenever $||\bx-\bx_0||<\delta$. If $f(\bx)\leq 0$ for some $\bx \in B_\delta(\bx_0)$, then $f(\bx)=-|f(\bx)|$ and Equation \eqref{eq:cont_condition} implies
	\begin{align*}
		|f(\bx)-f(\bx_0)|&=|-|f(\bx)|-f(\bx_0)| \\
		&=|-(\underbrace{|f(\bx)|+f(\bx_0)}_{>0})| \\
		&=|f(\bx)|+f(\bx_0)<\frac{f(\bx_0)}{2},
	\end{align*}
	which is a contradiction since $f(\bx_0)+|f(\bx)| \geq f(\bx_0)$. Hence, $f(\bx)$ must be positive throughout $B_\delta(\bx_0)$.
\end{proof}

\section{Level Curves of a Differentiable Function}
\label{sec:level_curves_of_a_differentiable_function}
\begin{thm}
Let $f:U \subseteq \R^2 \to \R$ be continuously differentiable on a connected subset $U$. If $\nabla f \neq 0$ at all points along the level curve $C=\{\bx \in U:f(\bx)=c\}$, then $C$ is continuously differentiable and $\nabla f$ is normal to $C$ at all points of $C$.
\end{thm}

\begin{proof}
	A little technical, so we omit the proof. This follows from the Implicit Function Theorem. 
\end{proof}
