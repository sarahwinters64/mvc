...
\begin{thm}[Implicit Function Theorem]\label{thm:implicit_function_thm}
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.4]{figures_mvc/differential_graph_illustration}
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.5]{figures_mvc/dirderiv}
...
\end{align*}
\begin{defn}[Linearization of a differentiable function]
	The function
\begin{align*}
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.5]{figures_mvc/change_in_f}
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.5]{figures_mvc/grad_lc}
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.5]{figures_mvc/tangent_space}
	\caption{The tangent plane and normal line at $P_0$.}
\end{figure}

\begin{defn}[Tangent Plane and Normal Line]\hspace{15cm}
	\begin{enumerate}[(a)]
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.5]{figures_mvc/tp_nl_ex}
...
\begin{figure}[h]
	\begin{center}
	\includegraphics[scale=0.7]{figures_mvc/evt_ex}
	\end{center}	
...
\begin{figure}[h]
	\begin{center}
	\includegraphics[scale=0.7]{figures_mvc/evt_non_ex}
	\end{center}	
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.7]{figures_mvc/horiz_tangent_line_fermat}
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.5]{figures_mvc/abs_max_min_example}
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.5]{figures_mvc/abs_max_min_exercise}
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.5]{figures_mvc/max_min_values}
	\caption{The graph of a multivariable function with various maxima and minima.}
\end{figure}

\begin{defn}
	A real-valued function of two variables $f$ has a \emph{local maximum} at $(a,b)$ if $f(x,y) \leq f(a,b)$ for all $(x,y)$ in some open set containing $(a,b)$. Similarly, $f$ has a \emph{local minimum} at $(a,b)$ if $f(x,y) \geq f(a,b)$ for all $(x,y)$ in some open set containing $(a,b)$.
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.5]{figures_mvc/gr_ex}
...
\end{align*}
Thus, on every open disc centered at $(0,0)$, $f$ takes positive and negative values, so $(0,0)$ is neither a local max nor a local min of $f$. We call a critical point with this property a \emph{saddle point}.
\end{example}

\begin{figure}[h]
	\centering
	\includegraphics[scale=0.5]{figures_mvc/saddle_pt}
	\caption{The graph of the function $f(x,y)=y^2-x^2$.}
\end{figure}

\begin{thm}[Second Derivative Test]
	Let $f$ be a function of two variables whose second partial derivatives of $f$ are continuous on a disk containing a critical point $(a,b)$ of $f$ and let $D$ denote the quantity
\begin{align*}
	D=D(a,b)=\frac{\partial^2f}{\partial^2 x}(a,b)\frac{\partial^2f}{\partial^2 y}(a,b)-\left[\frac{\partial^2f}{\partial x \partial y}(a,b)\right]^2.
\end{align*}
\begin{enumerate}[(a)]
		\item If $D>0$ and $\frac{\partial^2f}{\partial^2 x}(a,b)>0$, then $f(a,b)$ is a local minimum.
		\item If $D>0$ and $\frac{\partial^2f}{\partial^2 x}(a,b)<0$, then $f(a,b)$ is a local maximum.
		\item If $D<0$, then $f(a,b)$ is neither a local maximum nor minimum.
	\end{enumerate}
\end{thm}

\begin{remarks}\hspace{15cm}
\begin{enumerate}
	\item In case (c), $(a,b)$ is called a {\bf saddle point} of $f$ and the graph of $f$ crosses its tangent plane at $(a,b)$.
	\item If $D=0$, the test is inconclusive.
	\item $D$ can be written as $\begin{vmatrix}
		\frac{\partial^2 f}{\partial^2x} & \frac{\partial^2 f}{\partial x \partial y} \\
...
\begin{proof}
	We only prove part (a), as the proof of part (b) is similar. One can show that part (c) is inconclusive by considering different examples.
	\begin{enumerate}[(a)]
		\item Suppose $f_{xx}(a,b)>0$ and $D(a,b)>0$. We will show that there exists an open disc $B_\delta(a,b)$ such that $D^2_\bu(x,y)>0$ for all $(x,y) \in B_\delta(a,b)$, where $\bu=(h,k)$ is an arbitrary unit vector. This will show that the curve $C$, obtained by intersecting the graph of $f$ with the vertical plane through $P(a,b,f(a,b))$ in the direction of $\bu$ is concave upward on an interval of length $\delta>0$. Since $\bu$ is arbitrary, this is true in the direction of every unit vector, so for all $(x,y) \in B_\delta(x,y)$, the graph of $f(x,y)$ lies above its horizontal tangent plane $z=f(a,b)$, and therefore is a local minimum.
		 
		To see this, since $f_{xx}$ and $D=f_{xx}f_{yy}-(f_{xy})^2$ are continuous at $(a,b)$ with $f_{xx}(a,b)>0$ and $D(a,b)>0$, there exists $\delta>0$ such that $f_{xx}>0$ and $D>0$ for all $(x,y) \in B_\delta(a,b)$. \fixme{Justify this.} Now,
		\begin{align*}
			D^2_\bu f&=D_\bu(D_\bu f) \\
			&=D_\bu(f_xh+f_yk) \\
			&=(f_xh+f_yk)_xh+(f_xh+f_yk)_yk \\
			&=f_{xx}h^2+f_{yx}kh+f_{xy}hk+f_{yy}k^2 \\
			&=f_{xx}h^2+2f_{xy}hk+f_{yy}k^2 \\
			&=f_{xx}(h^2+2\frac{f_{xy}}{f_{xx}}hk)+f_{yy}k^2 \\
			&=f_{xx}[(h+\frac{f_{xy}}{f_{xx}}k)^2-\frac{f_{xy}^2}{f_{xx}^2}k^2]+f_{yy}k^2 \\
			&=f_{xx}(h+\frac{f_{xy}}{f_{xx}}k)^2+(f_{yy}-\frac{f_{xy}^2}{f_{xx}})k^2 \\
			&=\underbrace{f_{xx}}_{>0}\underbrace{(h+\frac{f_{xy}}{f_{xx}}k)^2}_{>0}+\underbrace{\frac{k^2}{f_{xx}}}_{>0}\underbrace{(f_{xx}f_{yy}-f_{xy}^2)}_{>0}>0.
		\end{align*}
	\end{enumerate} 
\end{proof}
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.5]{figures_mvc/pants}
...
{\color{red}
\begin{solution}
	\fixme{Explain why it suffices to minimize $d^2$.} The square of the distance from any point $(x,y,z)$ to $(1,0,-2)$ is
	\begin{align*}
		d^2=(x-1)^2+y^2+(z+2)^2.
	\end{align*}
...
\begin{thm}[Extreme value theorem for functions of two variables]
 If $f$ is continuous on a closed, bounded set $D$ in $\mathbb{R}^2$, then $f$ attains an absolute max $f(x_1,y_1)$ and an absolute min $f(x_2,y_2)$ at some points $(x_1,y_1)$ and $(x_2,y_2)$ in $D$.	
\end{thm}
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.5]{figures_mvc/closed_and_not_closed}
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.5]{figures_mvc/abs_ex_1}
\end{figure}	
\end{example}
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.5]{figures_mvc/abs_ex_2}
\end{figure}	
...
\begin{proof}
	(By contradiction.) Suppose $L>M$. Then
	\begin{align*}
		\lim_{x \to a}[g(x)-f(x)]=M-L.
	\end{align*}
	Since $L-M>0$ there exists $\delta>0$ such that
	\begin{align*}
		|[g(x)-f(x)]-(M-L)|<L-M
	\end{align*}
	whenever $0<|x-a|<\delta$. Since $a \leq |a|$ for any real number $a$, whenever $0<|x-a|<\delta$ we have
	\begin{align*}
		[g(x)-f(x)]-(M-L)&<L-M \\
		g(x)-f(x)&<0 \\
		g(x)<f(x),
	\end{align*}
	contradicting the assumption that $f(x) \leq g(x)$ for all such $0<|x-a|<\delta$. Thus, we conclude that $L \leq M$.
\end{proof}
