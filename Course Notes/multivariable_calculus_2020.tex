...
\begin{enumerate}[(a)]
	\item If $\bbr$ is a straight line, then $\bbr(s)=\bv(s_0)s+\bbr(s_0)$, where $\bbr(s_0), \bv(s_0)$ are constant vectors and we have $\kappa(s)=\ba(s)\equiv 0$. Conversely, if $\kappa=||\ba(s)||\equiv 0$, then integrating twice gives $\bbr(s)=\bv(s_0)s+\bbr(s_0)$, so the curve is a straight line.
...
\begin{defn}[Unit tangent and Unit normal in an arbitrary parametrization]
	...
	\begin{align*}
		\ut(t)=\frac{\bv(t)}{||\bbr'(t)||}, \hspace{0.5cm} \un(t)=\frac{\ba_\perp(t)}{||\ba_\perp(t)||}.
	\end{align*} 
\end{defn}
