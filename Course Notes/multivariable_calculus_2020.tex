...
\end{align}
\fixme{Add example.}
How do we compute the arc length if $\bbr(t)$ is not the graph of a function? If $\bbr(t)$ is not the graph of a function, it turns out that Equation \eqref{eq:arc_length_gen} is still valid. In this case, we obtain the formula as the limit of a sequence of polygonal approximations of the curve: 
...
\begin{prop}
Let $\bbr:[a,b] \to \R^n$ be a regular curve. Then the arc length between the points $\bbr(a)$ and $\bbr(b)$ on the curve is given by
\begin{align*}
	s=\int_a^b||\bbr'(t)||dt.
\end{align*}	
\end{prop}
